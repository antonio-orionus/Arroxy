<div align="center">
  <img src="src/renderer/src/assets/App-icon-HQ.png" alt="د Arroxy ماسکوټ" width="180" />

# Arroxy — د Windows، macOS او Linux لپاره وړیا خلاصه سرچینه YouTube ډاونلوډر

**4K · 1080p60 · HDR · MP3 · Shorts · Subtitles · SponsorBlock**

**پدې ژبه ولولئ:** [English](README.md) · [Español](README.es.md) · [Deutsch](README.de.md) · [Français](README.fr.md) · [日本語](README.ja.md) · [中文](README.zh.md) · [Русский](README.ru.md) · [Українська](README.uk.md) · [हिन्दी](README.hi.md) · [বাংলা](README.bn.md) · [العربية](README.ar.md) · [O'zbekcha](README.uz.md) · [မြန်မာဘာသာ](README.my.md) · **پښتو** · [Kiswahili](README.sw.md) · [አማርኛ](README.am.md) · [Afaan Oromoo](README.om.md) · [Ελληνικά](README.el.md) · [Српски](README.sr.md)

[![خپرونه](https://img.shields.io/github/v/release/antonio-orionus/Arroxy?label=Release&color=blueviolet)](https://github.com/antonio-orionus/Arroxy/releases/latest) [![جوړونه](https://img.shields.io/github/actions/workflow/status/antonio-orionus/Arroxy/release.yml?label=Build)](https://github.com/antonio-orionus/Arroxy/actions/workflows/release.yml) ![جواز](https://img.shields.io/badge/license-MIT-green) ![پلیټفارمونه](https://img.shields.io/badge/platform-cross--platform-1f2937?logo=github&logoColor=white) ![ژبې](https://img.shields.io/badge/i18n-18_languages-blue)

هر YouTube ویډیو، Short، یا اوډیو ټریک د اصلي کیفیت سره ډاونلوډ کړئ — تر 4K HDR پورې د 60 fps سره، یا د MP3 / AAC / Opus په توګه. د Windows، macOS، او Linux پر سیستم ځایي چلیږي. **هیڅ اعلانات، هیڅ ننوتل، هیڅ براوزر کوکیز، هیڅ د ګوګل اکاونټ لینک نه دی.**

[**↓ وروستۍ خپرونه ډاونلوډ کړئ**](../../releases/latest) &nbsp;·&nbsp; [Windows](#download) · [macOS](#download) · [Linux](#download)

<img src="build/demo.gif" alt="د Arroxy ډیمو" width="720" />

که Arroxy ستاسو وخت خوندي کوي، یو ⭐ نورو سره مرسته کوي چې ومومي.

</div>

---

## منځپانګه

- [ولې Arroxy](#why)
- [هیڅ کوکیز، هیڅ ننوتل، هیڅ د اکاونټ لینک کول](#no-cookies)
- [ځانګړتیاوې](#features)
- [ډاونلوډ](#download)
- [محرمیت](#privacy)
- [مکرر پوښتنې](#faq)
- [د پرمختیا لار](#roadmap)
- [د جوړولو ټیکنالوژي](#tech)

---

## <a id="why"></a>ولې Arroxy

د ترټولو عام بدیلونو سره د اړخ پر اړخ پرتله:

|                              | Arroxy | 4K Video Downloader | JDownloader | Y2Mate / online converters | Browser extensions |
| ---------------------------- | :----: | :-----------------: | :---------: | :------------------------: | :----------------: |
| وړیا، هیڅ پریمیم کچه نشته        |   ✅   |         ⚠️          |     ✅      |             ⚠️             |         ⚠️         |
| خلاصه سرچینه                  |   ✅   |         ❌          |     ❌      |             ❌             |         ⚠️         |
| یوازې ځایي پروسس کول        |   ✅   |         ✅          |     ✅      |             ❌             |         ✅         |
| هیڅ ننوتل یا د کوکیز صادرول نشته    |   ✅   |         ⚠️          |     ⚠️      |             ⚠️             |         ✅         |
| هیڅ د کارولو سقف نشته                |   ✅   |         ⚠️          |     ✅      |             🚫             |         ⚠️         |
| کراس-پلیټفارم ډیسکټاپ اپلیکیشن   |   ✅   |         ✅          |     ✅      |             N/A            |         ❌         |
| ساب ټایټلونه + SponsorBlock     |   ✅   |         ⚠️          |     ❌      |             ❌             |         ❌         |

Arroxy د یوه کار لپاره جوړ شوی: URL پیسټ کړئ، پاک ځایني فایل ترلاسه کړئ. هیڅ اکاونټونه، هیڅ اضافي پلورنه، هیڅ د معلوماتو راټولونه.

---

## <a id="no-cookies"></a>هیڅ کوکیز، هیڅ ننوتل، هیڅ د اکاونټ لینک کول

دا ترټولو عام لامل دی چې د ډیسکټاپ YouTube ډاونلوډرونه ماتیږي — او د Arroxy د شتون اصلي دلیل.

کله چې YouTube خپل بوټ کشف تازه کوي، ډیری وسیلې تاسو ته وايي چې د عارضي حل لپاره د خپل براوزر YouTube کوکیز صادر کړئ. دوه ستونزې پدې کې شته:

1. صادر شوي سیشنونه معمولا د ~30 دقیقو دننه پای ته رسیږي، نو تاسو مکرر صادرونه کوئ.
2. د yt-dlp خپله اسناد [خبرداری ورکوي چې د کوکیز پر بنسټ اتوماسیون کولی شي ستاسو د ګوګل اکاونټ بیرغ ولوي](https://github.com/yt-dlp/yt-dlp/wiki/Extractors#exporting-youtube-cookies).

**Arroxy هیڅکله د کوکیزونو، ننوتلو، یا کوم اعتبار غوښتنه نه کوي.** دا یوازې هغه عامه ټوکنونه کاروي چې YouTube هر براوزر ته وړاندې کوي. ستاسو د ګوګل پیژندتیا سره هیڅ تړاو نشته، هیڅ پای ته رسیدو نشته، هیڅ بدلونونه نشته.

---

## <a id="features"></a>ځانګړتیاوې

### کیفیت او بڼه

- تر **4K UHD (2160p)** پورې، 1440p، 1080p، 720p، 480p، 360p
- **لوړ فریم نرخ** لکه چې دی ساتل کیږي — 60 fps، 120 fps، HDR
- **یوازې اوډیو** MP3، AAC، یا Opus ته صادرول
- ګړندي مخکینۍ: *ترټولو ښه کیفیت* · *متوازن* · *کوچنی فایل*

### محرمیت او کنټرول

- ۱۰۰٪ ځایني پروسس کول — ډاونلوډونه مستقیما له YouTube څخه ستاسو ډیسک ته ځي
- هیڅ ننوتل، هیڅ کوکیز، هیڅ د ګوګل اکاونټ لینک نه دی
- فایلونه مستقیما هغه فولډر ته خوندي کیږي چې تاسو انتخاب کوئ

### کاري جریان

- **هر YouTube URL پیسټ کړئ** — ویډیوز او Shorts دواړه ملاتړ کیږي
- **د ډیری ډاونلوډونو کیو** — ډیری ډاونلوډونه سره سره تعقیب کړئ
- **د کلپبورډ لیدل** — یو YouTube لینک کاپي کړئ او Arroxy د اپ بیا تمرکز کولو پر مهال URL پخپله ډکوي (د پرمختللو تنظیماتو کې بدل کړئ)
- **د URL اتوماتیک پاکول** — د تعقیب پیرامیترونه (`si`، `pp`، `utm_*`، `fbclid`، `gclid`) لرې کوي او `youtube.com/redirect` لینکونه پرانیستي
- **د ټرې حالت** — د کړکۍ بندول ډاونلوډونه شالید کې روان ساتي
- **۹ ژبې** — د سیستم ژبه اتوماتیک کشف کوي، هر وخت بدلولی شئ

### ساب ټایټلونه او د پروسس کولو وروسته

- **ساب ټایټلونه** د SRT، VTT، یا ASS کې — لاسي یا اتوماتیک جوړ شوي، د هر شته ژبې کې
- د ویډیو تر څنګ خوندي کړئ، `.mkv` کې ځای پر ځای کړئ، یا د `Subtitles/` فرعي فولډر کې سازمان ورکړئ
- **SponsorBlock** — سپانسران، شمولې، وتلو، ځان-سپارښتنې پریږدئ یا د فصل نښه ورکړئ
- **ځای پر ځای شوي میټاډاټا** — سرلیک، د اپلوډ نیټه، چینل، توضیح، تصویر، او د فصل نښانونه فایل ته لیکل کیږي

<div align="center">
  <img src="build/Main-screenshot.png" width="48%" alt="یو URL پیسټ کړئ" />
  <img src="build/Choosing-format-screenshot.png" width="48%" alt="خپل کیفیت غوره کړئ" />
  <br/>
  <img src="build/Choosing-destination-screenshot.png" width="48%" alt="د خوندي کولو ځای غوره کړئ" />
  <img src="build/Downloading-in-parallel-screenshot.png" width="48%" alt="د ډاونلوډ کیو عمل کې" />
  <br/>
  <img src="build/Subtitles-screenshot.png" width="48%" alt="د ساب ټایټل ژبه او بڼه انتخاب کوونکی" />
</div>

---

## <a id="download"></a>ډاونلوډ

| پلیټفارم | بڼه |
| -------- | ------ |
| Windows  | نصب کوونکی (NSIS) یا پورټیبل `.exe` |
| macOS    | `.dmg` (Intel + Apple Silicon) |
| Linux    | `.AppImage` یا `.flatpak` (sandboxed) |

[**وروستۍ خپرونه واخلئ →**](../../releases/latest)

### د پیکیج مدیر له لارې نصب کول

| چینل  | کمانډ |
| -------- | ------- |
| Winget   | `winget install AntonioOrionus.Arroxy` |
| Scoop    | `scoop bucket add arroxy https://github.com/antonio-orionus/scoop-bucket && scoop install arroxy` |
| Homebrew | `brew tap antonio-orionus/arroxy && brew install --cask arroxy` |

<details>
<summary><strong>Windows: نصب کوونکی د پورټیبل پرتله</strong></summary>

|                       | NSIS نصب کوونکی | پورټیبل `.exe` |
| --------------------- | :------------: | :-------------: |
| نصب کولو ته اړتیا ده |      هو       | نه — له هر ځای چلیږي |
| اتوماتیک تازه کول          |   ✅ د اپ دننه    |   ❌ لاسي ډاونلوډ |
| د پیل سرعت         |   ✅ ګړندي    |   ⚠️ ورو سړه پیل |
| د پیل مینو ته اضافه کوي    |      ✅        |       ❌        |
| اسانه لرې کول        |      ✅        |   ❌ فایل حذف کړئ |

**سپارښتنه:** د اتوماتیک تازه کولو او ګړندي پیل لپاره د NSIS نصب کوونکی وکاروئ. د بې نصبه، بې ریجسټرۍ اختیار لپاره پورټیبل `.exe` وکاروئ.

</details>

<details>
<summary><strong>د macOS پر لومړي پیل</strong></summary>

Arroxy لاهم کوډ نه دی لاسلیک شوی، نو macOS Gatekeeper به تاسو ته د لومړي پیل پر مهال خبرداری درکوي. دا متوقع دی — دا د زیان نښه نه ده.

**د سیستم تنظیماتو طریقه (سپارښتنه شوې):**

1. د Arroxy اپ آیکون ښي کلیک وکړئ او **Open** غوره کړئ.
2. د خبرداری ډیالوګ ښکاري — **Cancel** کلیک وکړئ (د *Move to Trash* کلیک مه کوئ).
3. **System Settings → Privacy & Security** خلاص کړئ.
4. د **Security** برخې ته سکرول وکړئ. تاسو به وګورئ *"Arroxy was blocked from use because it is not from an identified developer."*
5. **Open Anyway** کلیک وکړئ او د خپل پاسورډ یا Touch ID سره تایید کړئ.

د ۵مې ګام وروسته، Arroxy نورمال خلاصیږي او خبرداری بیا نه ښکاري.

**د ټرمینل طریقه (پرمختللي):**

```bash
xattr -dr com.apple.quarantine /Applications/Arroxy.app
```

> د macOS جوړونه د Apple Silicon او Intel رنرانو پر CI کیږي. که تاسو ستونزو سره مخ شئ، مهرباني وکړئ [یوه ستونزه خلاصه کړئ](../../issues) — د macOS کارونکو بازخورد فعاله د macOS ازموینې دورې بڼه ورکوي.

</details>

<details>
<summary><strong>د Linux پر لومړي پیل</strong></summary>

AppImages مستقیم چلیږي — هیڅ نصب کول نشته. تاسو یوازې اړتیا لرئ فایل د اجرا وړ نښه کړئ.

**د فایل مدیر:** د `.AppImage` ښي کلیک وکړئ → **Properties** → **Permissions** → **Allow executing file as program** فعال کړئ، بیا دوه ځله کلیک وکړئ.

**ټرمینل:**

```bash
chmod +x Arroxy-*.AppImage
./Arroxy-*.AppImage
```

که پیل لاهم ناکام شي، ممکن تاسو ته FUSE کمه وي:

```bash
# Ubuntu / Debian
sudo apt install -y libfuse2

# Fedora
sudo dnf install -y fuse-libs

# Arch
sudo pacman -S fuse2
```

**Flatpak (sandboxed بدیل):** د ورته خپرونې پاڼې څخه `Arroxy-*.flatpak` ډاونلوډ کړئ.

```bash
flatpak install --user Arroxy-*.flatpak
flatpak run io.github.antonio_orionus.Arroxy
```

</details>

---

## <a id="privacy"></a>محرمیت

ډاونلوډونه مستقیما د [yt-dlp](https://github.com/yt-dlp/yt-dlp) له لارې له YouTube څخه هغه فولډر ته راوړل کیږي چې تاسو انتخاب کوئ — هیڅ شی د دریمې ډلې سرور له لارې نه تیریږي. د لیدلو تاریخ، ډاونلوډ تاریخ، URL، او د فایل منځپانګه ستاسو وسیله کې پاتې کیږي.

Arroxy د [Aptabase](https://aptabase.com) له لارې ناپیژاندل شوې، ټولیزه ټیلیمیټري لیږي — یوازې د یوه مستقل پروژه لپاره د دې لیدلو لپاره کافي چې ایا چا واقعیا کاروي (پیلونه، OS، د اپ نسخه، کریشونه). هیڅ URL، هیڅ د ویډیو سرلیکونه، هیڅ د فایل لارې، هیڅ IP، هیڅ د اکاونټ معلومات — Aptabase د ډیزاین له مخې خلاصه سرچینه او GDPR-دوستانه ده. تاسو د تنظیماتو کې لاسپورته کیدای شئ.

---

## <a id="faq"></a>مکرر پوښتنې

**ایا دا واقعیا وړیا ده؟**
هو — د MIT جواز، هیڅ پریمیم کچه، هیڅ د ځانګړتیا محدودیت نشته.

**کومې ویډیو کیفیتونه ډاونلوډ کولی شم؟**
هر هغه شی چې YouTube وړاندې کوي: 4K UHD (2160p)، 1440p، 1080p، 720p، 480p، 360p، او یوازې اوډیو. 60 fps، 120 fps، او HDR سټریمونه لکه چې دي ساتل کیږي.

**ایا کولی شم یوازې اوډیو د MP3 په توګه وباسم؟**
هو. د بڼې مینو کې *یوازې اوډیو* غوره کړئ او MP3، AAC، یا Opus غوره کړئ.

**ایا ما ته د YouTube اکاونټ یا کوکیز ته اړتیا ده؟**
نه. Arroxy یوازې هغه عامه ټوکنونه کاروي چې YouTube هر براوزر ته وړاندې کوي. هیڅ کوکیز، هیڅ ننوتل، هیڅ اعتبار خوندي شوی. د لامل لپاره [هیڅ کوکیز، هیڅ ننوتل، هیڅ د اکاونټ لینک کول](#no-cookies) وګورئ.

**ایا کله چې YouTube یو شی بدل کوي کار کوي؟**
د استحکام دوه پوړونه: yt-dlp د YouTube بدلونونو وروسته د ساعتونو دننه تازه کیږي، او Arroxy هغو کوکیزونو پورې اړه نه لري چې هره ~30 دقیقه پای ته رسیږي. دا دا هغو وسیلو پرتله نمایان ثابت ورکوي چې د صادر شوو براوزر سیشنونو پورې اړه لري.

**Arroxy د کومو ژبو لپاره شته دی؟**
نهه: English، Español، Deutsch، Français، 日本語، 中文، Русский، Українська، हिन्दी. ستاسو د سیستم ژبه اتوماتیک کشف کوي؛ د ټول بار له لارې هر وخت بدلول. د ژبې فایلونه د `src/shared/i18n/locales/` کې ساده TypeScript اعتراضونه دي — [PRs ښه راغلاست](../../pulls).

**ایا ما ته بل شی نصب کولو ته اړتیا ده؟**
نه. yt-dlp او ffmpeg د لومړي پیل پر مهال اتوماتیک له دوی رسمي GitHub خپرونو ډاونلوډ او ځایي ذخیره کیږي.

**ایا کولی شم پلیلیسټونه یا ټول چینلونه ډاونلوډ کړم؟**
نن ورځ یوازې یو ویډیوز او Shorts. د پلیلیسټ او چینل ملاتړ د [د پرمختیا لار](#roadmap) پر مهال دی.

**macOS وايي "اپ خرابه ده" — څه وکړم؟**
دا macOS Gatekeeper دی چې بې لاسلیکه اپ بندوي، واقعي زیان نه دی. د حل لپاره [د macOS پر لومړي پیل](#download) برخه وګورئ.

**ایا د YouTube ویډیوز ډاونلوډ کول قانوني دي؟**
د شخصي، خصوصي کارولو لپاره دا معمولا ډیرو قضایي سیمو کې منل کیږي. تاسو د YouTube د [د کارولو شرایطو](https://www.youtube.com/t/terms) او د خپل ځایني د کاپي رایټ قوانینو سره مطابقت لرل ستاسو مسؤلیت دی.

---

## <a id="roadmap"></a>د پرمختیا لار

راتلونکې — تقریبا د لومړیتوب ترتیب کې:

| ځانګړتیا | توضیح |
| ------- | ----------- |
| **د پلیلیسټ او چینل ډاونلوډونه** | یو پلیلیسټ یا چینل URL پیسټ کړئ؛ ټول ویډیوز د نیټې یا شمیر فیلترونو سره کیو کړئ |
| **د بیچ URL ننوتل** | یو وخت ډیری URL پیسټ کړئ او دوی یوه ځل چلوئ |
| **د بڼې بدلون** | ډاونلوډونه د جلا وسیلې پرته MP3، WAV، FLAC ته بدل کړئ |
| **د دودیز فایل نوم ټیمپلیټونه** | فایلونه د سرلیک، اپلوډ کوونکي، نیټه، ریزولیوشن له مخې نوم ورکړئ — د ژوندۍ پریویو سره |
| **مهالویش شوي ډاونلوډونه** | کیو د ټاکلي وخت کې پیل کړئ (شپه اجرا کول) |
| **د سرعت محدودیتونه** | بانډویډث محدود کړئ چې ډاونلوډونه ستاسو اتصال نه پوروي |
| **د کلپ کمول** | یوازې د پیل/پای وخت له مخې یو برخه ډاونلوډ کړئ |

ذهن کې کومه ځانګړتیا لرئ؟ [یوه غوښتنه خلاصه کړئ](../../issues) — د ټولنې ننوتل لومړیتوب ورکوي.

---

## <a id="tech"></a>د جوړولو ټیکنالوژي

<details>
<summary><strong>ټیک سټیک</strong></summary>

- **Electron** — د ډیسکټاپ کراس-پلیټفارم شیل
- **React 19** + **TypeScript** — کارونکي انٹرفیس
- **Tailwind CSS v4** — ستایل کول
- **Zustand** — د حالت مدیریت
- **yt-dlp** + **ffmpeg** — د ډاونلوډ او mux انجن (د لومړي پیل پر مهال له GitHub څخه راوړل کیږي، تل تازه)
- **Vite** + **electron-vite** — د جوړولو وسیلې
- **Vitest** + **Playwright** — د واحد او له سره تر سره ازموینې

</details>

<details>
<summary><strong>له سرچینې جوړول</strong></summary>

### مخکینۍ شرطونه — ټولې پلیټفارمونه

| وسیله | نسخه | نصب |
| ---- | ------- | ------- |
| Git  | هر     | [git-scm.com](https://git-scm.com) |
| Bun  | وروستۍ  | لاندې د OS سره سم وګورئ |

### Windows

```powershell
powershell -c "irm bun.sh/install.ps1 | iex"
```

د اصلي جوړولو وسیلې ته اړتیا نشته — پروژه هیڅ اصلي Node اضافه نه لري.

### macOS

```bash
xcode-select --install
curl -fsSL https://bun.sh/install | bash
```

### Linux (Ubuntu / Debian)

```bash
curl -fsSL https://bun.sh/install | bash

# Electron د چلولو وابستګۍ
sudo apt install -y libgtk-3-0 libnss3 libasound2t64

# یوازې E2E ازموینې (Electron ته ښکاره کیدو ته اړتیا ده)
sudo apt install -y xvfb
```

### کلون کول او چلول

```bash
git clone https://github.com/antonio-orionus/Arroxy
cd arroxy
bun install
bun run dev          # د ګرم ریلوډ dev جوړونه
```

### د توزیع وړ جوړول

```bash
bun run build        # د ډول ازموینه + کمپایل
bun run dist         # د اوسني OS لپاره پیکیج کول
bun run dist:win     # د Windows پورټیبل exe کراس-کمپایل
```

> yt-dlp او ffmpeg شامل نه دي — دوی د لومړي پیل پر مهال له دوی رسمي GitHub خپرونو څخه ډاونلوډ او د app ډیټا فولډر کې خوندي کیږي.

</details>

---

## د کارولو شرایط

Arroxy یوازې د شخصي، خصوصي کارولو لپاره یوه وسیله ده. تاسو یوازې مسؤل یاست چې ستاسو ډاونلوډونه د YouTube د [د کارولو شرایطو](https://www.youtube.com/t/terms) او ستاسو د قضایي سیمې د کاپي رایټ قوانینو سره مطابقت لري. د Arroxy د ډاونلوډ، تکرار، یا د هغه منځپانګې توزیع لپاره مه کاروئ چې تاسو حق نه لرئ وکاروئ. پراختیا کونکي د هر ناسم کارولو لپاره مسؤل نه دي.

<div align="center">
  <sub>MIT License · د مینې سره جوړ شوی <a href="https://x.com/OrionusAI">@OrionusAI</a> لخوا</sub>
</div>
