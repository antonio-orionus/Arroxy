...
- **23 языков** — автоматически определяет язык системы, можно переключить в любой момент
