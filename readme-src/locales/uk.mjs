...
export const uk = {
  ...
  feat_workflow_12: "**Глобальна гаряча клавіша завантаження** — скопіюйте посилання в будь-якому застосунку й натисніть `Ctrl+Shift+D` (`Cmd+Shift+D` на macOS); Arroxy додасть його до черги з вашим активним профілем, не відкриваючи вікно, а сповіщення це підтвердить. Увімкнено за замовчуванням, комбінацію можна змінити",
  feat_workflow_1:
  ...
  hotkey_fig_alt: "Глобальна гаряча клавіша завантаження Arroxy — Ctrl+Shift+D у Windows і Linux, Cmd+Shift+D у macOS, надсилає скопійоване посилання просто до черги завантажень",
  hotkey_fig_cap: "<b>Глобальна гаряча клавіша завантаження</b><br/>Скопіюйте посилання будь-де, натисніть один раз — воно потрапляє в чергу й починає завантажуватися",
  ...
  dl_linux_fuse_text: "Якщо запуск все одно не вдається — запустіть без монтування, пакунок FUSE не потрібен:",
  dl_linux_targz_h4: "Звичайний архів (без FUSE, без встановлення):",
  dl_linux_targz_text: "Збірка `.tar.gz` — той самий застосунок без оболонки AppImage: розпакуйте будь-куди й запустіть. Ані інсталятора, ані пакунка FUSE.",
  dl_linux_flatpak_prereq: "В Ubuntu замість Flatpak постачається Snap, тож спершу встановіть Flatpak і додайте Flathub — звідти пакунок завантажує свій runtime:",
  dl_linux_arch_note: "**Завантаження для Linux на сторінці релізів лише x86_64.** На машинах ARM64 (Raspberry Pi, Asahi Linux) Flatpak встановиться, але не запуститься: `bwrap: execvp ldconfig: Exec format error`.",
