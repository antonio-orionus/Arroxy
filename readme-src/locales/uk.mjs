...
export const uk = {
  ...
  discord_badge_text: "Приєднатися до Discord-спільноти",
  discord_badge_encoded:
    "%D0%9F%D1%80%D0%B8%D1%94%D0%B4%D0%BD%D0%B0%D1%82%D0%B8%D1%81%D1%8F%20%D0%B4%D0%BE%20Discord%2D%D1%81%D0%BF%D1%96%D0%BB%D1%8C%D0%BD%D0%BE%D1%82%D0%B8",
  hero_desc:
  ...
  feat_workflow_6:
    "**23 мови** — автоматично визначає локаль системи, яку можна будь-коли перемикати",
  feat_workflow_7:
  ...
  shot1_cap:
    "<b>Головна швидкого завантаження</b><br/>Вставте URL і одразу завантажте з активним профілем",
  shot2_cap:
    "<b>Багаторазові профілі завантаження</b><br/>Зберігайте формат, якість і вивід як пресети — використовуйте для кожного завантаження",
  shot3_cap:
    "<b>Багатомовні аудіодоріжки</b><br/>Виберіть саме ту мову звуку, що є у відео",
  shot4_cap:
    "<b>Об'ємний / Dolby звук</b><br/>Доріжки 5.1 і Dolby розпізнаються та зберігаються",
  shot5_cap:
    "<b>Режим масових URL</b><br/>Вставте список, авто-вилучення дублікатів, поставте все в чергу одразу",
  shot6_cap:
    "<b>Паралельна черга завантажень</b><br/>Кілька завантажень одночасно з живим прогресом",
  ...
  faq_a6:
    "Двадцять три, з коробки: English, Bahasa Indonesia (індонезійська), Español (іспанська), Deutsch (німецька), Français (французька), 日本語 (японська), 中文 (китайська), Русский (російська), українська (українська), हिन्दी (гінді), Afaan Oromoo, Kiswahili, O'zbekcha (узбецька), Tiếng Việt (в’єтнамська), Türkçe, አማርኛ (амхарська), العربية (арабська), اردو (урду), پښتو (пушту), বাংলা (бенгальська), မြန်မာဘာသာ (бірманська), Ελληνικά (грецька) та Српски (сербська). Arroxy автоматично визначає мову вашої операційної системи під час першого запуску, і ви можете будь-коли переключитися за допомогою засобу вибору мови на панелі інструментів. Локаль Runtime JSON знаходиться в src/shared/i18n/locales/, а каталоги PO для перекладачів знаходяться в i18n/locales/ — відкрийте PR на GitHub, щоб зробити внесок.",
  ...
  plan_intro: "Досі заплановано — приблизно за пріоритетом:",
  plan_col1: "Функція",
  plan_col2: "Опис",
  plan_r1_name: "**Фільтри плейлистів і каналів**",
  plan_r1_desc: "Date-range filters під час переліку playlist або channel",
  plan_r2_name: "**Пріоритети аудіодоріжок YouTube**",
  plan_r2_desc:
    "Задати бажану мовну аудіодоріжку для всього застосунку, із перевизначеннями в окремих профілях, коли YouTube надає кілька аудіотреків",
  plan_r6_name: "**Вхід через браузер у застосунку**",
  plan_r6_desc:
    "Відкривати браузерні вікна всередині Arroxy, щоб входити на сайти й використовувати cookies без ручного експорту",
  plan_r8_name: "**Завантаження відео в один клік**",
  plan_r8_desc:
    "Запускати завантаження відео в один клік із виявленого або вставленого URL за активним профілем",
  plan_r3_name: "**Надійніше відновлення повтором**",
  plan_r3_desc:
    "Новий шлях повторної спроби для завантажень, перерваних нестабільним або проблемним інтернет-з'єднанням",
  plan_r4_name: "**Повноцінний менеджер завантажень у drawer**",
  plan_r4_desc:
    "Перетворити drawer черги на повніший менеджер, включно зі зміною папок призначення для елементів у черзі",
