...
export const ru = {
  ...
  dl_h2: "Установка и первый запуск",
  ...
  feat_workflow_12: "**Глобальная горячая клавиша загрузки** — скопируйте ссылку в любом приложении и нажмите `Ctrl+Shift+D` (`Cmd+Shift+D` на macOS); Arroxy добавит её в очередь с вашим активным профилем, не открывая окно, а уведомление это подтвердит. Включено по умолчанию, сочетание можно изменить",
  feat_workflow_1:
  ...
  feat_workflow_6:
    "**Языки: {{LANG_COUNT}}** — автоматически определяет язык системы, можно переключить в любой момент",
  feat_workflow_7:
  ...
  hotkey_fig_alt: "Глобальная горячая клавиша загрузки Arroxy — Ctrl+Shift+D в Windows и Linux, Cmd+Shift+D в macOS, отправляет скопированную ссылку прямо в очередь загрузок",
  hotkey_fig_cap: "<b>Глобальная горячая клавиша загрузки</b><br/>Скопируйте ссылку где угодно и нажмите один раз — она попадёт в очередь и начнёт скачиваться",
  shot7_cap: "<b>Профили для каждого видео плейлиста</b><br/>Своя настройка для каждого видео — часть в 4K, остальные как MP3",
  ...
  dl_macos_warning:
    "Arroxy пока не подписан кодом, поэтому macOS Gatekeeper при первом запуске может показать предупреждение о повреждённом приложении. Это ожидаемо — файлы не повреждены на самом деле.",
  dl_macos_m1_h4: "Способ через Terminal:",
  dl_macos_step1:
    "Перетащите `Arroxy.app` из смонтированного DMG в `/Applications`.",
  dl_macos_step2:
    "Откройте Terminal и выполните `sudo xattr -dr com.apple.quarantine /Applications/Arroxy.app`.",
  dl_macos_step3:
    "Выполните `open /Applications/Arroxy.app`.",
  dl_macos_step4:
    "Если путь к приложению другой, замените `/Applications/Arroxy.app` на путь, куда вы его установили.",
  dl_macos_step5:
    "Введите пароль Mac, если `sudo` его запросит.",
  dl_macos_after:
    "После снятия карантина Arroxy откроется нормально.",
  dl_macos_m2_h4: "Способ через Terminal:",
  dl_macos_note:
  ...
  dl_linux_fuse_text: "Если запуск всё равно не удаётся — запустите без монтирования, пакет FUSE не нужен:",
  dl_linux_targz_h4: "Обычный архив (без FUSE, без установки):",
  dl_linux_targz_text: "Сборка `.tar.gz` — то же приложение без обёртки AppImage: распакуйте куда угодно и запустите. Ни установщика, ни пакета FUSE.",
  dl_linux_flatpak_prereq: "В Ubuntu вместо Flatpak поставляется Snap, поэтому сначала установите Flatpak и добавьте Flathub — оттуда пакет загружает свой рантайм:",
  dl_linux_arch_note: "**Загрузки для Linux на странице релизов только x86_64.** На машинах ARM64 (Raspberry Pi, Asahi Linux) Flatpak установится, но не запустится: `bwrap: execvp ldconfig: Exec format error`.",
  ...
  dl_macos_sequoia_h4: "Исправление через Terminal для актуального macOS",
  dl_macos_sequoia_intro:
    "Используйте Terminal после копирования Arroxy в Applications:",
  dl_macos_sequoia_step1:
    "Перетащите `Arroxy.app` из смонтированного DMG в `/Applications`.",
  dl_macos_sequoia_step2:
    "Откройте Terminal и выполните две команды:",
  dl_macos_sequoia_step3:
    "Выполните `open /Applications/Arroxy.app`, чтобы запустить Arroxy.",
  dl_macos_sequoia_step4:
    "Если путь к приложению другой, замените `/Applications/Arroxy.app` на путь, куда вы его установили.",
  dl_macos_sonoma_h4: "Исправление через Terminal для старых macOS",
  dl_macos_sonoma_step1:
    "Перетащите `Arroxy.app` из смонтированного DMG в `/Applications`.",
  dl_macos_sonoma_step2:
    "Откройте Terminal и снимите карантин с `/Applications/Arroxy.app`.",
  dl_macos_sonoma_step3:
    "Запустите Arroxy из Terminal или Finder после снятия карантина.",
  dl_macos_damaged_h4:
    "Исправление карантина Gatekeeper",
  dl_macos_damaged_p:
