...
- **QueueResumeLifecycle** — main-process lifecycle module for resumable media retries. Owns media evidence from yt-dlp progress, retry eligibility, tempDir preservation/cleanup, resume tempDir validation, and QueueStore restart projection. `src/main/services/download/QueueResumeLifecycle.ts`.
...
- **BinaryDownloader** — HTTP fetch with range-resume, checksum helpers, and download-error classification. Pure I/O, no provider policy. Exposes `parseShaLine`/`parseStandaloneSha256`/`parsePowerShellFileHash`, `sha256ForFile`, `downloadText`, `downloadFile`, and `classifyDownloadError`. RuntimeBinaryMaterializer calls it for approved manifest artifacts. `src/main/services/binary/BinaryDownloader.ts`.
...
- **QuickDownloadPreparation** — renderer run lifecycle for Quick Download and bulk Quick Download. Owns run ids, cancellation, probe sequencing, output fallback, playlist cap handling, partial failure counts, active-profile queue submission, queue-tip UI, and reset. `src/renderer/src/store/wizard/quickDownloadPreparation.ts`.
- **DownloadProfile** — main-screen Quick Download profile used by the profile picker/active-profile flow. Built-in profiles live separately from interactive wizard presets. `src/shared/downloadProfiles.ts`.
- **DownloadProfileAction** — renderer action model for active DownloadProfile controls. Projects active profile, summary text, icon, menu labels, and `DownloadProfileRef` selection once for Home and Bulk URL adapters. `src/renderer/src/components/wizard/downloadProfileActions.ts`.
...
- **FixtureMediaCatalog** — JSON-backed catalog for deterministic Fixture Product E2E media facts: video ids, titles, playlist membership, format descriptors, media body kind, content type, and file sizes. TypeScript harness and Python extractor act as adapters over this catalog. `tests/e2e/fixture-media-catalog.json`, `tests/e2e/fixtureMediaCatalog.ts`.
...
`scripts/test-binaries/smoke-all.sh` validates upstream binary source availability for runtime manifest automation and embedded ffmpeg/ffprobe fetches. Run before bumping fetch-embedded.sh asset names or runtime manifest source facts. **`scripts/test-binaries/` directory must stay tracked** — an earlier `.gitignore` rule `test-binaries/` accidentally matched it. Now anchored: `**/build/embedded/` (build output) and `/test-binaries/` (smoke output, root only).
