...
- Runtime dependency: `@oxlint/plugins`, pinned exactly to Arroxy's `oxlint` version.
...
This directory is root-owned lint infrastructure, not a workspace package. `src/`
is copied from the upstream commit above and is excluded from Arroxy's root
lint/format/typecheck file allowlists except when invoked by the Oxlint bridge.
