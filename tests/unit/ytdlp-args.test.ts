...
describe('YtDlp — probe args', () => {
	...
	it("playlistMode='video' ignores playlist scope args", async () => {
		await makeYtDlp().run({kind: 'probe', url: URL, playlistMode: 'video', playlistScope: {items: {kind: 'range', from: 10, to: 20}}})
		const args = getArgs()
		expect(args).toContain('--no-playlist')
...
describe('YtDlp — outputTemplate', () => {
	it('video kind: outputTemplate replaces default %(title)s.%(ext)s', async () => {
		await makeYtDlp().run({kind: 'video', url: URL, outputDir: OUTPUT_DIR, outputTemplate: '01 - %(title)s.%(ext)s'})
		const args = getArgs()
	...
	it('subtitle kind: outputTemplate honored', async () => {
		await makeYtDlp().run({kind: 'subtitle', url: URL, outputDir: OUTPUT_DIR, subtitleLanguages: ['en'], subtitleFormat: 'srt', outputTemplate: '07 - %(title)s.%(ext)s'})
		const args = getArgs()
		const oArg = args[args.indexOf('-o') + 1]
	...
	it('video kind: tempDir + outputTemplate keeps -o template-only', async () => {
		await makeYtDlp().run({kind: 'video', url: URL, outputDir: OUTPUT_DIR, tempDir: '/tmp/dl', outputTemplate: '02 - %(title)s.%(ext)s'})
		const args = getArgs()
...
describe('YtDlp — video args', () => {
	...
	it('with formatSelector → -f <selector>, formatId ignored', async () => {
		await makeYtDlp().run({kind: 'video', url: URL, outputDir: OUTPUT_DIR, formatId: '137', formatSelector: 'bestvideo[height<=1080]+bestaudio/best[height<=1080]'})
		const args = getArgs()
		const fIdx = args.indexOf('-f')
		expect(fIdx).toBeGreaterThan(-1)
	...
	it('skipDownload → --skip-download present, no -f', async () => {
		await makeYtDlp().run({kind: 'video', url: URL, outputDir: OUTPUT_DIR, formatSelector: 'bestaudio/best', skipDownload: true})
		const args = getArgs()
...
describe('YtDlp — audio convert args', () => {
	it('mp3 192K → -f bestaudio/best, -x, --audio-format mp3, --audio-quality 192K', async () => {
		await makeYtDlp().run({kind: 'video', url: URL, outputDir: OUTPUT_DIR, audioConvert: {target: 'mp3', bitrateKbps: 192}})
		const args = getArgs()
	...
	it('wav → no --audio-quality', async () => {
		await makeYtDlp().run({kind: 'video', url: URL, outputDir: OUTPUT_DIR, audioConvert: {target: 'wav'}})
		const args = getArgs()
	...
	it('explicit embedThumbnail=false overrides auto-embed for music format', async () => {
		await makeYtDlp().run({kind: 'video', url: URL, outputDir: OUTPUT_DIR, audioConvert: {target: 'mp3', bitrateKbps: 192}, embedThumbnail: false, embedMetadata: false})
		const args = getArgs()
		expect(args).not.toContain('--embed-thumbnail')
	...
	it('opus 128K threads bitrate correctly', async () => {
		await makeYtDlp().run({kind: 'video', url: URL, outputDir: OUTPUT_DIR, audioConvert: {target: 'opus', bitrateKbps: 128}})
		const args = getArgs()
	...
	it('m4a 256K threads bitrate correctly', async () => {
		await makeYtDlp().run({kind: 'video', url: URL, outputDir: OUTPUT_DIR, audioConvert: {target: 'm4a', bitrateKbps: 256}})
		const args = getArgs()
	...
	it('audioConvert overrides any provided formatId', async () => {
		await makeYtDlp().run({kind: 'video', url: URL, outputDir: OUTPUT_DIR, formatId: 'bv+ba', audioConvert: {target: 'mp3', bitrateKbps: 192}})
		const args = getArgs()
		expect(args[args.indexOf('-f') + 1]).toBe('bestaudio/best')
...
describe('YtDlp — video+embed args', () => {
	it('--continue present', async () => {
		await makeYtDlp().run({kind: 'video+embed', url: URL, outputDir: OUTPUT_DIR, subtitleLanguages: ['en']})
		expect(getArgs()).toContain('--continue')
	})

	it('with subs → embed subtitle flags and EMBED_CONTAINER_EXT', async () => {
		await makeYtDlp().run({kind: 'video+embed', url: URL, outputDir: OUTPUT_DIR, subtitleLanguages: ['en', 'ja'], writeAutoSubs: false})
		const args = getArgs()
	...
	it('writeAutoSubs=true → adds --write-auto-subs', async () => {
		await makeYtDlp().run({kind: 'video+embed', url: URL, outputDir: OUTPUT_DIR, subtitleLanguages: ['en'], writeAutoSubs: true})
		expect(getArgs()).toContain('--write-auto-subs')
	})

	it('empty subs array → falls back to no-subs branch', async () => {
		await makeYtDlp().run({kind: 'video+embed', url: URL, outputDir: OUTPUT_DIR, subtitleLanguages: []})
		const args = getArgs()
		expect(args).toContain('--no-write-subs')
		expect(args).toContain('--no-write-auto-subs')
...
describe('YtDlp — subtitle args', () => {
	...
	it('writeAutoSubs=true → adds --write-auto-subs', async () => {
		await makeYtDlp().run({kind: 'subtitle', url: URL, outputDir: OUTPUT_DIR, subtitleLanguages: ['en'], subtitleFormat: 'srt', writeAutoSubs: true})
		expect(getArgs()).toContain('--write-auto-subs')
	})

	it('subtitleMode=subfolder → output path under <dir>/subtitles/', async () => {
		await makeYtDlp().run({kind: 'subtitle', url: URL, outputDir: OUTPUT_DIR, subtitleLanguages: ['en'], subtitleFormat: 'srt', subtitleMode: 'subfolder'})
		const args = getArgs()
	...
	it('subtitleMode=sidecar → output path is directly in outputDir (no subfolder)', async () => {
		await makeYtDlp().run({kind: 'subtitle', url: URL, outputDir: OUTPUT_DIR, subtitleLanguages: ['en'], subtitleFormat: 'srt', subtitleMode: 'sidecar'})
	...
	it('ass + writeAutoSubs → coerced to srt in args, effectiveSubtitleFormat=srt on result', async () => {
		const result = await makeYtDlp().run({kind: 'subtitle', url: URL, outputDir: OUTPUT_DIR, subtitleLanguages: ['en'], subtitleFormat: 'ass', writeAutoSubs: true})
		const args = getArgs()
	...
	it('ass without writeAutoSubs → no coercion', async () => {
		await makeYtDlp().run({kind: 'subtitle', url: URL, outputDir: OUTPUT_DIR, subtitleLanguages: ['en'], subtitleFormat: 'ass', writeAutoSubs: false})
		const args = getArgs()
...
describe('YtDlp — output embed flags', () => {
	...
	it('sponsorBlock.mode=mark + embedChapters=false → no --embed-chapters (setting fully owns the flag)', async () => {
		await makeYtDlp().run({kind: 'video', url: URL, outputDir: OUTPUT_DIR, embedChapters: false, sponsorBlock: {mode: 'mark', categories: ['sponsor']}})
		expect(getArgs()).not.toContain('--embed-chapters')
	})

	it('sponsorBlock.mode=mark + embedChapters undefined → no --embed-chapters', async () => {
		await makeYtDlp().run({kind: 'video', url: URL, outputDir: OUTPUT_DIR, sponsorBlock: {mode: 'mark', categories: ['sponsor']}})
		expect(getArgs()).not.toContain('--embed-chapters')
	})

	it('sponsorBlock.mode=mark + embedChapters=true → --embed-chapters present', async () => {
		await makeYtDlp().run({kind: 'video', url: URL, outputDir: OUTPUT_DIR, embedChapters: true, sponsorBlock: {mode: 'mark', categories: ['sponsor']}})
		expect(getArgs()).toContain('--embed-chapters')
	})
	...
	it('embedThumbnail=true on kind=video+embed with subs → flags absent (MKV-forced)', async () => {
		await makeYtDlp().run({kind: 'video+embed', url: URL, outputDir: OUTPUT_DIR, subtitleLanguages: ['en'], embedThumbnail: true})
		expect(getArgs()).not.toContain('--embed-thumbnail')
		expect(getArgs()).not.toContain('--convert-thumbnails')
	})

	it('embedThumbnail=true on kind=video+embed with empty subs → flags PRESENT (no MKV-force)', async () => {
		await makeYtDlp().run({kind: 'video+embed', url: URL, outputDir: OUTPUT_DIR, subtitleLanguages: [], embedThumbnail: true})
		const args = getArgs()
		expect(args).toContain('--embed-thumbnail')
		expect(args[args.indexOf('--convert-thumbnails') + 1]).toBe('jpg')
	})
...
describe('YtDlp — SponsorBlock args', () => {
	...
	it('mark mode adds --sponsorblock-mark without implying chapter embedding', async () => {
		await makeYtDlp().run({kind: 'video', url: URL, outputDir: OUTPUT_DIR, sponsorBlock: {mode: 'mark', categories: ['sponsor', 'selfpromo']}})
	...
	it('remove mode adds --sponsorblock-remove without implying chapter embedding', async () => {
		await makeYtDlp().run({kind: 'video', url: URL, outputDir: OUTPUT_DIR, sponsorBlock: {mode: 'remove', categories: ['sponsor', 'intro', 'outro']}})
	...
	it('empty categories omit SponsorBlock args', async () => {
		await makeYtDlp().run({kind: 'video', url: URL, outputDir: OUTPUT_DIR, sponsorBlock: {mode: 'mark', categories: []}})
...
describe('YtDlp — sidecar flags', () => {
	...
	it('writeDescription=true on kind=video+embed → --write-description present', async () => {
		await makeYtDlp().run({kind: 'video+embed', url: URL, outputDir: OUTPUT_DIR, subtitleLanguages: ['en'], writeDescription: true})
		expect(getArgs()).toContain('--write-description')
	})

	it('writeThumbnail=true on kind=video+embed → --write-thumbnail present', async () => {
		await makeYtDlp().run({kind: 'video+embed', url: URL, outputDir: OUTPUT_DIR, subtitleLanguages: ['en'], writeThumbnail: true})
		expect(getArgs()).toContain('--write-thumbnail')
	})
