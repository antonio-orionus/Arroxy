...
import { nextMonotonicPercent, ProgressSmoother } from '@renderer/store/progress';
...
describe('progress ui helpers', () => {
  ...
  it('extracts speed and eta details from download lines', () => {
    const s = new ProgressSmoother();
    // First valid line: no EWMA blending (initializes directly), no throttle (lastEmitTime=0)
    expect(s.update('[download] 54.0% of 18.53MiB at 3.49MiB/s ETA 00:02')).toBe(
      '3.49MiB/s • ETA 0:02'
    );
    // Unknown B/s after a valid reading: preserves last detail
    const s2 = new ProgressSmoother();
    expect(s2.update('[download] 72.7% of 18.53MiB at Unknown B/s ETA Unknown')).toBeNull();

    const s3 = new ProgressSmoother();
