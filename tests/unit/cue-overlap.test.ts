...
describe('normalizeCueOverlaps', () => {
	...
	// "Looks like a timing line" does not make it a cue. A NOTE block is free
	// text and may describe a range; rewriting it corrupts metadata.
	it('never rewrites a timing-shaped line inside a NOTE block', () => {
		const input = ['WEBVTT', '', 'NOTE', '00:00:00.000 --> 00:00:10.000 chapter marker', '', '00:00:05.000 --> 00:00:09.000', 'real cue', ''].join('\n')
	...
	// Payload lines follow the cue's own timing line, so a subtitle that opens
	// with a timestamp range must be left as displayed text.
	it('never rewrites a timing-shaped line inside cue payload', () => {
		const input = ['1', '00:00:00,000 --> 00:00:03,000', '00:00:01,000 --> 00:00:02,000 is when it happened', '', '2', '00:00:01,000 --> 00:00:04,000', 'next', ''].join('\n')

		const {content, clamped} = normalizeCueOverlaps(input)

		expect(clamped).toBe(1)
		// The real cue end moved; the payload line is untouched.
		expect(content).toContain('00:00:00,000 --> 00:00:01,000\n00:00:01,000 --> 00:00:02,000 is when it happened')
	})

	it('still normalizes cues that follow a metadata block', () => {
		const input = ['WEBVTT', '', 'NOTE', '00:00:00.000 --> 00:00:10.000 chapter marker', '', '00:00:00.000 --> 00:00:03.320', 'first', '', '00:00:01.760 --> 00:00:04.960', 'second', ''].join('\n')

		const {content, clamped} = normalizeCueOverlaps(input)

		expect(clamped).toBe(1)
		expect(content).toContain('00:00:00.000 --> 00:00:01.760')
		expect(content).toContain('00:00:00.000 --> 00:00:10.000 chapter marker')
	})
