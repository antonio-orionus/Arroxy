...
describe('startDownloadSchema — multi-site URL acceptance', () => {
	...
	it('accepts non-YouTube URLs (multi-site)', () => {
		const result = startDownloadSchema.safeParse({url: 'https://vimeo.com/12345', job: {kind: 'single-format', ...IDENTITY, formatId: '137', preset: 'custom', sponsorBlock: {mode: 'off'}, embed: BASE_EMBED}})
		expect(result.success).toBe(true)
	})

	it('rejects non-http/https URLs', () => {
		const result = startDownloadSchema.safeParse({url: 'ftp://example.com/file.mp4', job: {kind: 'single-format', ...IDENTITY, formatId: '137', preset: 'custom', sponsorBlock: {mode: 'off'}, embed: BASE_EMBED}})
		expect(result.success).toBe(false)
	})

	it('accepts subtitle-only job with languages', () => {
		const result = startDownloadSchema.safeParse({url: 'https://www.youtube.com/watch?v=dQw4w9WgXcQ', job: {kind: 'subtitle-only', ...IDENTITY, subtitles: {languages: ['en', 'en-US', 'pt-BR', 'en-orig'], mode: 'sidecar', format: 'srt', writeAuto: false}}})
		expect(result.success).toBe(true)
	})
...
describe('infoDictSchema — null normalization + _type discrimination', () => {
	it('treats null filesize/fps/duration as undefined (not validation errors)', () => {
		const raw = {title: 'x', thumbnail: null, duration: null, formats: [{format_id: '22', filesize: null, fps: null, abr: null, ext: 'mp4'}], subtitles: null, automatic_captions: null}
	...
	it('passes through unknown fields without rejecting', () => {
		const raw = {title: 'x', _unknown_field_yt_dlp_added_in_2030: 'whatever', formats: [{format_id: '22', _new_codec_field: 'av2'}]}
	...
	it('discriminates on _type: multi_video', () => {
		const raw = {_type: 'multi_video', id: 'episode-1', title: 'Multi-act episode', entries: [{id: 'a', title: 'Act 1', url: 'https://x.com/a'}]}
		expect(infoDictSchema.safeParse(raw).success).toBe(true)
	})
