...
describe('strategyFor — kind routing', () => {
	...
	it('ranged-format with no subtitles → video', () => {
		const job: PreparedJob = {kind: 'ranged-format', extractor: 'youtube', extractorKey: 'Youtube', intent: {kind: 'video-audio', codec: 'best', tiers: ['best'], audio: {format: 'best'}}, filenameTemplate: '%(title)s.%(ext)s', sponsorBlock: SB_OFF, embed: EMBED_OFF}
		expect(strategyFor(job)).toBe<StrategyKind>('video')
	})
...
describe('phasesFor — production repro cell', () => {
	...
	it('audio-only + m4a-convert + 2 subs → preflight + video phase', () => {
		const job: PreparedJob = {kind: 'audio-convert', extractor: 'youtube', extractorKey: 'Youtube', audioConvert: {target: 'm4a', bitrateKbps: 192}, preset: 'audio-only', sponsorBlock: SB_OFF, embed: EMBED_OFF, subtitles: {languages: ['en-j3PyPqV-e1s', 'en-orig'], mode: 'sidecar', format: 'srt', writeAuto: false}}
