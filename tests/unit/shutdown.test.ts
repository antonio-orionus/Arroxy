...
function makeDeps(queueCancelResult: Result<CancelOutput> = ok({cancelled: true})) {
	return {queueService: {cancel: vi.fn().mockResolvedValue(queueCancelResult)}, tokenService: {dispose: vi.fn()}, logInfo: vi.fn(), exit: vi.fn()}
}
...
describe('cancelQueueBeforeExit', () => {
	...
	it('still disposes tokens and exits if queue cancellation throws', async () => {
		const deps = makeDeps()
		deps.queueService.cancel.mockRejectedValue(new Error('boom'))

		await cancelQueueBeforeExit(deps)

		expect(deps.logInfo).toHaveBeenCalledWith('Queue cancellation before shutdown failed', {error: 'boom'})
		expect(deps.tokenService.dispose).toHaveBeenCalledOnce()
		expect(deps.exit).toHaveBeenCalledWith(0)
	})
})
