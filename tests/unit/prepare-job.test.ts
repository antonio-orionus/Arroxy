  ...
  describe('single-format kind', () => {
    ...
    it('attaches subtitles when languages non-empty', () => {
      const job = prepareJob({
        ...
        formatId: '22',
        activePreset: null,
        subtitles: { languages: ['en', 'ja'], mode: 'sidecar', format: 'srt', writeAuto: false }
      });
      if (job.kind !== 'single-format') throw new Error('unreachable');
    ...
    it('drops empty subtitles bundle', () => {
      const job = prepareJob({
        ...
        formatId: '22',
        activePreset: null,
        subtitles: { languages: [], mode: 'sidecar', format: 'srt', writeAuto: false }
      });
      if (job.kind !== 'single-format') throw new Error('unreachable');
  ...
  describe('subtitle-only kind', () => {
    ...
    it('triggers via implicit "subs only, no media" intent', () => {
      const job = prepareJob({
        ...
        mode: 'single',
        activePreset: null,
        subtitles: { languages: ['en'], mode: 'sidecar', format: 'srt', writeAuto: true }
      });
