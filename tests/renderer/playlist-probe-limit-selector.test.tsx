...
function entries(count: number): PlaylistEntry[] {
	return Array.from({length: count}, (_, index) => {
		const n = index + 1
		return {id: `p${n}`, url: `https://youtube.com/watch?v=p${n}`, title: `Video ${n}`, thumbnail: '', playlistIndex: n, videoId: `p${n}`}
	})
}
...
function playlistProbe(count: number): Extract<ProbeResult, {kind: 'playlist'}> {
	return {kind: 'playlist', extractor: 'youtube:playlist', extractorKey: 'YoutubePlaylist', webpageUrl: PLAYLIST_URL, isAudioOnlySource: false, playlistTitle: 'Capped Playlist', playlistId: 'PLcap', isMultiVideo: false, entries: entries(count)}
}
...
function installApi(): AppApi {
	...
	api.settings.update = vi.fn(async (input: SettingsPatch) => {
		...
		return ok({common: {...current.common, ...(input.common ?? {})}, single: {...current.single, ...(input.single ?? {})}, playlist: {...current.playlist, ...(input.playlist ?? {})}, profiles: {...current.profiles, ...(input.profiles ?? {})}})
	})
