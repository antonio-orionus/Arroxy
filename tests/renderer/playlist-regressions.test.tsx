...
function buildMockApi(settingsOverrides: Record<string, unknown> = {}) {
	return {
		...
		downloads: {
			...
			probe: vi.fn().mockResolvedValue(ok({kind: 'playlist' as const, extractor: 'youtube:tab', extractorKey: 'YoutubeTab', webpageUrl: 'https://www.youtube.com/playlist?list=PL123', isMultiVideo: false, playlistId: 'PL123', playlistTitle: 'Playlist', entries: PLAYLIST_ENTRIES})),
...
describe('playlist regressions', () => {
	...
	it('playlist probe restores persisted common prefs before the first playlist save', async () => {
		...
		const api = buildMockApi({embedChapters: true, embedMetadata: true, embedThumbnail: true, writeDescription: true, writeThumbnail: true, lastSponsorBlockMode: 'mark', lastSponsorBlockCategories: ['intro'], lastPlaylistSelection: selAudioMp3, lastSubfolderEnabled: true, lastSubfolder: 'Saved Folder'})
		window.appApi = api as never
		...
		expect(patch.common).toMatchObject({embedChapters: true, embedMetadata: true, embedThumbnail: true, writeDescription: true, writeThumbnail: true, lastSponsorBlockMode: 'mark', lastSponsorBlockCategories: ['intro']})
	})
