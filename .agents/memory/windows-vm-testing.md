...
- (2026-09-14, issue #222 repro — **still applied**) Hosts file blocks `www.youtube.com` / `youtube.com` / `m.youtube.com` (lines tagged `# issue222`), and `%APPDATA%\arroxy\settings.json` has `cookiesMode: browser`, `cookiesBrowser: firefox`, `proxyUrl: 192.168.212.1:18080` (a throwaway proxy on the Mac, `/tmp/issue222/proxy.mjs`). Backups: `settings.json.issue222-*.bak` beside it and `C:\Users\admin\hosts.issue222-*.bak`. Arroxy 0.4.13 portable at `C:\Users\admin\arroxy-0413\`, launched into the desktop session via scheduled task `arroxy-issue222` with `--remote-debugging-port=9333`. If YouTube "doesn't load" on the VM, this is why — remove the tagged hosts lines and restore the settings backup when #222 is done.
