...
function videoIntentSpec(intent: VideoMediaIntent): MediaIntentSpec {
  ...
  if (tier === 'best') {
    ...
    if (intent.codec === 'mp4') {
      return {
        formatSelector: preferM4aAudio ? 'bestvideo*+bestaudio[ext=m4a]/bestvideo*+bestaudio/best' : 'bestvideo*+bestaudio/best',
    ...
    return {
      formatSelector: preferM4aAudio ? 'bestvideo*+bestaudio[ext=m4a]/bestvideo*+bestaudio/best' : 'bestvideo*+bestaudio/best',
      formatSort: preferM4aAudio ? 'acodec:m4a' : undefined,
      producesVideo: true,
      producesAudio: true
    };
  }
  ...
  return {
    formatSelector: preferM4aAudio ? `bestvideo[height<=${h}]+bestaudio[ext=m4a]/bestvideo[height<=${h}]+bestaudio/best[height<=${h}]` : `bestvideo[height<=${h}]+bestaudio/best[height<=${h}]`,
    formatSort: preferM4aAudio ? 'acodec:m4a' : undefined,
    producesVideo: true,
    producesAudio: true
  };
