      ...
      <div className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-2.5 items-center -mx-1">
        ...
        <div role="radiogroup" aria-label={t('wizard.sponsorblock.modeHeading')} className="flex flex-row flex-wrap gap-1">
          {SPONSORBLOCK_MODES.map((mode) => (
            <RadioOption
              key={mode}
              label={t(SB_MODE_LABEL_KEYS[mode])}
              checked={wizardSponsorBlockMode === mode}
              onClick={() => setSponsorBlockMode(mode)}
              className="py-0.5"
            />
          ))}
        </div>
        ...
        <p className="text-[11px] text-[var(--text-subtle)] leading-snug">
          {t(SB_MODE_HINT_KEYS[wizardSponsorBlockMode])}
        </p>
      </div>
      ...
      {showCategories && (
        <>
          ...
          <div data-testid="sb-categories">
            <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-subtle)] px-2 pt-1 pb-1.5">
              {t('wizard.sponsorblock.categoriesHeading')}
            </p>
              ...
              {SPONSORBLOCK_CATEGORIES.map((cat) => {
                ...
                return (
                  <label
                    ...
                    className="flex w-full items-center gap-2 h-7 px-2 rounded-md text-sm font-medium transition-colors cursor-pointer hover:bg-accent/60 has-[[data-checked]]:bg-[var(--brand-dim)] has-[[data-checked]]:border-l-2 has-[[data-checked]]:border-[var(--brand)] has-[[data-checked]]:text-[var(--brand)]"
                  >
                    <Checkbox
                      checked={isChecked}
                      onCheckedChange={() => toggleSponsorBlockCategory(cat)}
                      className="border-[var(--border-strong)] data-checked:border-[var(--brand)] data-checked:bg-[var(--brand)] data-checked:text-white"
                    />
                    <span className="flex-1 text-left truncate">
                      {t(`wizard.sponsorblock.cat.${cat}`)}
                    </span>
                  </label>
                );
              })}
