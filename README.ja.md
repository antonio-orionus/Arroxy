...
- **同時ダウンロード数と自動再試行** — キューのダウンロードを同時に何件走らせるか選べます。一時的な失敗は待ち時間を延ばしながら自動で再試行されます
